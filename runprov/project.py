...
# `git` is absent because it was WITHDRAWN on 2026-08-20 — the collision with GitPython's
# top-level module and the 'swallow everything, return None, 20s timeout' contract were the
# reasons, and `__init__.py` records them. It stays reachable as `runprov.project.git`.
# `default_run_id` and `default_generation` are WITHDRAWN and stay reachable here too.
# (This paragraph used to argue that the question was "still open" and that "this file
# cannot take it" — which was true when written and false five lines above an `__all__`
# that no longer contained the name. ADR-0003's addendum records the decision as taken.)
__all__ = [
    "DEFAULT_CODE_PATHS",
    "DEFAULT_TRACKED",
    "Project",
