...
def _timeline(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        out.append(f"     code       {r.get('git_commit','?')}"
...
def _yaml(rows: list[dict]) -> str:
    ...
    out = ["# GENERATED by `python -m runprov log --format yaml`. Do not hand-edit —",
    ...
    for r in rows:
        ...
        if r.get("inputs"):
            ...
            for i in r["inputs"]:
                out.append(f"    - {q(str(i.get('sha256') or 'MISSING')[:64] + '  ' + str(i.get('path','')))}")
        if r.get("outputs"):
            out.append("  output_sha256:")
            for o in r["outputs"]:
                out.append(f"    - {q(str(o.get('sha256') or 'MISSING')[:64] + '  ' + str(o.get('path','')))}")
        out.append("")
    return "\n".join(out)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not path.is_file():
        print(f"no run history at {path}\n"
              ...
              f"and worth telling apart.", file=sys.stderr)
        return 1
    ...
    print(f"# {len(rows)} of {total} run(s) from {path}"
          + (f"; {n_failed} FAILED" if n_failed else "")
          + (f"; {bad} unreadable line(s) skipped" if bad else ""), file=sys.stderr)
    return 0
