...
# Nothing here is promised. This module is reached through `python -m runprov show`, and
# `staleness` is the one name a caller might plausibly want programmatically — it stays
# unpromised because its shape (the `rehash=` keyword, the `dict[str, str]` return, the
# state vocabulary) has just moved once under L-23 and may move again.
__all__: list[str] = []
...
def _yaml_entry(r: dict[str, typing.Any], *, separator: bool = True) -> str:
    ...
    THE BLANK LINE LEADS THE ENTRY rather than trailing it, and that is not a matter of
    taste: a trailing one is emitted after the LAST entry too, so the file ends with a blank
    line and the banner is no longer separated from the first record. Splitting this out of
    `_yaml` moved the separator to the end and changed both, silently -- a streaming writer
    cannot know it is on the last record, but it always knows whether it has written
    anything yet, so the same separation costs nothing to place correctly (L-74).
    """
