...
def test_verify_does_not_read_a_whole_binary_looking_for_a_pin(tmp_path):
    ...
    art.write_bytes(
        b"\x00" * (runprov.verify.SCAN_BYTES + 64) + runprov.hashing.PIN_ANCHOR.encode()
    )
...
def test_verify_treats_an_undecodable_file_as_unpinned(tmp_path):
    ...
    latin.write_bytes(f"# {runprov.hashing.PIN_ANCHOR}\n".encode("cp1252"))  # em dash -> 0x97
...
def test_the_pin_anchor_exists_exactly_once_in_the_source():
    ...
    `ANCHOR`. Writer and reader, with nothing holding them equal.

    Drift in either direction makes every artifact ever written stop being recognised as
    one — and silently, because an unrecognised pin is not an error, it is `NO PIN`.

    WHY A SOURCE-LEVEL ASSERTION AND NOT A BEHAVIOURAL ONE. The suite already catches the
    drift: measured, changing the em dash to a hyphen in the writer alone fails 20 tests,
    because end-to-end tests write a real pin and read it back. But every one of those tests
    builds its fixture from the READER's constant, so they prove the round trip agrees with
    itself — never that a second literal has not reappeared beside it. Only counting the
    definitions can say that, and the duplication is precisely what `PIN_SIDECAR_SUFFIX`'s
    own docstring warns against two lines above where `PIN_ANCHOR` now lives: "Two copies of
    the string would be the same defect deferred."""
    # THE SENTENCE ITSELF, not a quote-delimited literal. The first version of this test
    # searched for `"<sentence>"` with the quotes attached and did not fail when the writer's
    # copy came back, because that copy lives inside an f-string — `f"{c}provenance — …"` —
    # where a `{c}` sits between the opening quote and the first word. It passed over the
    # exact mutation it was written for.
    pkg = pathlib.Path(runprov.__file__).parent
    found = {
        f.name: f.read_text(encoding="utf-8").count(runprov.hashing.PIN_ANCHOR)
        for f in sorted(pkg.glob("*.py"))
    }
    found = {k: v for k, v in found.items() if v}
    assert found == {"hashing.py": 1}, (
        f"the pin anchor should appear once, in hashing.py beside PIN_DIGEST_CHARS and "
        f"PIN_SIDECAR_SUFFIX. Found: {found}"
    )
